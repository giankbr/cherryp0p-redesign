...
export function NewsGrid() {
  ...
  return (
    <section className="py-16 bg-black text-white dark:bg-black dark:text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border mb-px">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border">
            {articles.slice(1).map((article, index) => (
              <div key={index} className="bg-black dark:bg-black p-6">
                <div className="h-full flex flex-col">
                  <p className="text-xs text-primary mb-2">{article.date}</p>
                  <h3 className="text-xl font-bold mb-4 text-white">{article.title}</h3>
                  <p className="text-sm text-gray-200 mb-6 flex-grow">{article.excerpt}</p>
